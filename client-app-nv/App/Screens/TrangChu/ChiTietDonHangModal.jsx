                ...
                <View style={styles.modalContent}>
                    ...
                    >
                        <View style={styles.box}>
                            ...
                            <Text><Text style={styles.boldText}>Tổng tiền:</Text> {order.tongTien} VNĐ</Text>
                        ...
                        <View style={styles.box}>
                            ...
                            <View style={styles.tableContainer}>
                                <View style={styles.tableRow}>
                                    <Text style={[styles.tableHeader, { flex: 3 }]}>Thời gian bắt đầu</Text>
                                    <Text style={[styles.tableHeader, { flex: 3 }]}>Thời gian kết thúc</Text>
                                    <Text style={[styles.tableHeader, { flex: 3 }]}>Trạng thái lịch</Text>
                                </View>
                                {order.danhSachLichThucHien.map((lich, index) => (
                                    <View key={index} style={styles.tableRow}>
                                        <Text style={[styles.tableCell, { flex: 3 }]}>{new Date(lich.thoiGianBatDauLich).toLocaleString()}</Text>
                                        <Text style={[styles.tableCell, { flex: 3 }]}>{new Date(lich.thoiGianKetThucLich).toLocaleString()}</Text>
                                        <Text style={[styles.tableCell, { flex: 3 }]}>{lich.trangThaiLich}</Text>
                                    </View>
                                ))}
                            </View>
                        </View>
                        <View style={styles.box}>
                            ...
                            <View style={styles.tableContainer}>
                                <View style={styles.tableRow}>
                                    <Text style={[styles.tableHeader, { flex: 3 }]}>Loại dịch vụ</Text>
                                    <Text style={[styles.tableHeader, { flex: 3 }]}>Tên dịch vụ</Text>
                                    <Text style={[styles.tableHeader, { flex: 2 }]}>Thời gian (giờ)</Text>
                                    <Text style={[styles.tableHeader, { flex: 2 }]}>Giá</Text>
                                </View>
                                {order.danhSachDichVu.map((dichVu, index) => (
                                    <View key={index} style={styles.tableRow}>
                                        <Text style={[styles.tableCell, { flex: 3 }]}>{dichVu.loaiDichVu}</Text>
                                        <Text style={[styles.tableCell, { flex: 3 }]}>{dichVu.tenDichVu}</Text>
                                        <Text style={[styles.tableCell, { flex: 2 }]}>{dichVu.thoiGian}</Text>
                                        <Text style={[styles.tableCell, { flex: 2 }]}>{dichVu.gia} VNĐ</Text>
                                    </View>
                                ))}
                            </View>
                        </View>
