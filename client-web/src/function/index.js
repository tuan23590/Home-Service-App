...
export const EPOCHTODATETIMETOTIME = (epochDateStart, epochDateEnd) => {
    ...
    const start = formatDate(epochDateStart);
    const end = formatDate(epochDateEnd);

    return `${start.hours}:${start.minutes} đến ${end.hours}:${end.minutes} ${start.dayOfWeek}, ${start.day}/${start.month}/${start.year}`;
};
